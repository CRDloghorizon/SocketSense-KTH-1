...
if __name__ == '__main__':
    ...
    b = 5
    name = 'BME'
    train = dataloader('D:/Univariate_arff/' + name + '/' + name + '_TRAIN.arff')
    test = dataloader('D:/Univariate_arff/' + name + '/' + name + '_TEST.arff')
    data = np.vstack((train[:,:-1], test[:,:-1]))
    label = np.hstack((train[:, -1], test[:, -1]))
    ch = data.shape[1]

    som1 = SOMdtw(width=2, height=3, channel=ch, sigma = 1.0, learning_rate=0.5, w=w, b=b)
    # dm = 1 : dtw; dm = 2 : Euc
    som1.train(data=data, n_iteration=2, batch_size=10, dm=2)
    ...
    #plt.savefig('C:/Users/loghorizon/Desktop/thesis/scri6.png')
    plt.show()